...
function update_grid() {
  for (var i = 0; i < grid.length; i++) {
    for (var j = 0; j < grid[i].length; j++) {
      grid[i][j] = update_point([i * point_width, j * point_width], get_surrounding(i, j));
    }
  }
}

function update_point(square_position, surrounding) {
  var new_x = 0;
  var new_y = 0;
  ...
  new_x = max(new_x, square_position[0]);
  new_y = new_y / surrounding.length;
  new_y = max(new_y, square_position[1]);
  return [new_x, new_y];
}
